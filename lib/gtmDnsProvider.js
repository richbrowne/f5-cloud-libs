...
function GtmDnsProvider(options) {
    ...
    this.bigIp = new BigIp({logger: options.logger, loggerOptions: options.loggerOptions});
}
 ...
 * @param {String} providerOptions.host                - BIG-IP GTM management IP to which to send commands.
 * @param {String} providerOptions.user                - BIG-IP GTM admin user name.
 * @param {String} providerOptions.port                - BIG-IP GTM management SSL port to connect to. Default 443.
 * @param {String} providerOptions.password            - BIG-IP GTM admin user password. Use this or passwordUrl.
 * @param {String} providerOptions.passwordUrl         - URL (file, http(s)) to location that contains BIG-IP GTM admin user password. Use this or password.
 * @param {String} [providerOptions.passwordEncrypted] - Indicates that the BIG-IP GTM password is encrypted (either with encryptDataToFile or generatePassword).
 ...
 * @param {String} [providerOptions.vsMonitor]         - Full path to monitor for the virtual server. Default is existing monitor.
 * @param {String} [providerOptions.poolMonitor]       - Full path to monitor for the pool. Default is existing monitor.
 * @param {String} [providerOptions.loadBalancingMode] - Load balancing mode for the pool. Default is existing load balancing mode.
 ...
 */
GtmDnsProvider.prototype.init = function(providerOptions) {
 ...
 *                    elected master.
 */
GtmDnsProvider.prototype.update = function(instances) {
    ...
    .then(function() {
        ...
        return this.bigIp.gtm.updatePool(this.providerOptions.poolName, this.providerOptions.serverName, instances, options);
    }.bind(this));
};
